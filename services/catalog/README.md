...
| GET | `/v1/events?mine=true` | Requires a tenant; the caller's own events only, at any status (organizer dashboard) |
...
| POST | `/v1/events/{id}/publish` | Publish a draft event (tenant from JWT; 404s on a tenant mismatch) |
