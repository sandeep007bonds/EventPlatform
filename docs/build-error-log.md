...
| **CS8323** | The same `Ticket.Create` change again, in `TicketTests` — five calls still passed seven arguments, which also left `seatId:` in the wrong slot, and a named argument out of position followed by a positional one is what the compiler actually reports. **Not covered**: `check_static_factory_arity` bails on any call containing a named argument. An argument *count* is still comparable when names are present even though positions are not, so this is closable — it was left open deliberately rather than adding a sixth rule in a round where three of the previous five were miscalibrated on first write |
| **SA1507** | Two blank lines left in `QueueEndpoints` where the dead-letter registration was inserted with its own trailing blank | Two blank lines in a row. Nothing ambiguous about it, and the tree held exactly one instance |
| **CA1859** | A test helper capturing NSubstitute's arguments was declared `private IReadOnlyList<GeneralAdmissionAllocation> CapturedPools() => … .ToList();`. The interface reads like good hygiene and is the opposite here: the member is private, so the compiler sees every caller, and the abstraction only adds an indirection over a `List` the body just built | A `private`/`internal` expression-bodied member declared as a collection *interface* whose body ends in `.ToList()`/`.ToArray()`. **`Task<IReadOnlyList<T>>` is excluded**: CA1859 does not look through the task, and the repo's two async helpers returning one compile today — a rule flagging them would be wrong about the build rather than strict about it |
| **SA1118** | A refusal message too long for one line was split with `+` and handed straight to a `SaveSeatMapLayoutResult` constructor. An argument may not span lines; a local may, so the fix is always to name it and pass the name | A `+` continuation line with a `(` still open at that point — the concatenation is inside an argument list rather than on the right of an `=`. **Only this shape**, not SA1118 in general: the rule exempts lambdas, anonymous methods and initializers, so every `.Select(x => …).ToList()` passed to a constructor in this tree is a legal multi-line argument, and telling those from a violation needs to know what the argument *is* |
