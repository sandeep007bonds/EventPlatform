...
it applied, and exits. In Kubernetes that is an Argo CD Sync-phase hook job at a negative sync-wave,
so the schema lands before any new pod rolls.

That job is `deploy/base/<service>/migrate-job.yaml`, one per database-owning service, annotated
`argocd.argoproj.io/hook: Sync` + `argocd.argoproj.io/sync-wave: "-5"` (see the correction in
Consequences for why not `PreSync`). It runs the service's own image with `args: ["--migrate"]` and
...
- On the **first ever** sync into an empty namespace, the jobs run before any Deployment pod has
  mounted the `eventplatform-keyvault` SecretProviderClass — and that mount is what creates the
  `eventplatform-secrets` Secret their `secretKeyRef`s read. The jobs mount the class themselves,
...
- **Correction, found on the first real deploy: the hook phase above was originally `PreSync`, and
  that deadlocked.** The paragraph above reasoned correctly that the *Secret* materialises during
  the job's own mount, but stopped one level short: the `SecretProviderClass` **resource** is a
  normal manifest in `deploy/overlays/dev/`, and PreSync runs before every normal resource in the
  Application. So on a fresh namespace the hook waited for a volume whose SecretProviderClass the
  main sync had not created, and the main sync never began because the hook never finished — every
  migrate pod stuck in `ContainerCreating` with `FailedMount ... SecretProviderClass
  "eventplatform-keyvault" not found`, Application `OutOfSync`/`Missing`, indefinitely. The jobs are
  now `hook: Sync` at `sync-wave: "-5"`, with the SecretProviderClass at `-10` and everything else
  at the default `0`. Same guarantee — schema applied and waited for before any new pod rolls — with
  no cycle. The class stays a normal resource rather than becoming a hook itself, because hook
  resources default to `hook-delete-policy: BeforeHookCreation` and would be deleted and recreated
  on every sync, underneath the pods already mounting them.
- **Neither CI nor any test could have caught this.** `kustomize build` proves the manifests are
  well-formed; it says nothing about Argo CD's phase and wave ordering, which only exists at sync
  time against a real cluster. This class of defect needs a real deploy — a standing gap, recorded
  in `docs/system-reference.md`'s gap table.
